...
local function runPythonScript(testscript, arg1)
    local command = testscript .. " " .. arg1  
...
-- ((0, 0, 1000, 1, 2, 0, 0, 0)) produces 2 seconds frame as total.
if (ar1.FrameConfig(0, 0, 1000, 1, 2, 0, 0, 0) == 0) then
...
for count = 564, 1630 do
    ...
    for _, consonant in ipairs(consonants) do
        ...
        adc_data_path = "D:\\consonanats_correct\\"..consonant.. "\\adc_data_" ..count.. ".bin" 
		...
		print("-----------------------------------------------------")
		print("------------------------------6 SECONDS TO GET STARTED DOSYA " ..count.. "-----------------------")
		RSTD.Sleep(2000)
        runPythonScript(pythonScript, count)
